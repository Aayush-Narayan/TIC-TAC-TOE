...
const checkForWinner = (playerArr) => {
    return winningPatterns.some((pattern) => pattern.every((cell) => playerArr.includes(cell)));
};
function isDraw() {
    return arr.every((cellId) => document.querySelector(cellId).textContent ===
        "X" ||
        document.querySelector(cellId).textContent ===
            "O");
}
const resetBoard = () => {
    for (let val of arr) {
